...
namespace GSM09200FRONT
{
    ...
    {
        [Inject] private R_ILocalizer<GSM09200FrontResources.Resources_Dummy_Class> _localizer { get; set; }

        private GSM09200ViewModel loExpenditureCategoryViewModel = new();

        private R_Conductor _conductorExpenditureCategoryRef;

        private R_TreeView<GSM09200DTO> _treeExpenditureCategoryRef;
        ...
        private R_TextBox _expenditureIdRef;

        private R_TextBox _expenditureNameRef;
        ...
        private async Task PropertyDropdown_ValueChanged(string poParam)
        {
            ...
            try
            {
                ...
                await _treeExpenditureCategoryRef.R_RefreshTree(null);
        ...
        private async Task Grid_ExpenditureCategory_Display(R_DisplayEventArgs eventArgs)
        {
            ...
            {
                await _expenditureNameRef.FocusAsync();
            }
        }

        private async Task Tree_ExpenditureCategory_R_ServiceGetListRecord(R_ServiceGetListRecordEventArgs eventArgs)
        {
            R_Exception loEx = new R_Exception();
        ...
        private void Tree_R_RefreshTreeViewState(R_RefreshTreeViewStateEventArgs eventArgs)
        {
            ...
            try
            {
                List<GSM09200DTO> loTreeList = (List<GSM09200DTO>)eventArgs.TreeViewList;
                loTreeList.ForEach(x => x.LHAS_CHILD = loTreeList.Where(y => y.CPARENT == x.CCATEGORY_ID).Count() > 0 ? true : false);
                eventArgs.ExpandedList = loExpenditureCategoryViewModel.loExpenditureCategoryList.Where(x => x.LHAS_CHILD == true).ToList();
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
        ...
        private async Task Grid_ExpenditureCategory_AfterAdd(R_AfterAddEventArgs eventArgs)
        {
            ...
            try
            {
                await _expenditureIdRef.FocusAsync();
        ...
        private void Grid_ExpenditureCategory_ConvertToGridEntity(R_ConvertToGridEntityEventArgs eventArgs)
        {
            GSM09200DetailDTO loConductorData = (GSM09200DetailDTO)eventArgs.Data;
            eventArgs.GridData = new GSM09200DTO
            {
                CPARENT = loConductorData.CPARENT,
                CCATEGORY_ID = loConductorData.CCATEGORY_ID,
                CCATEGORY_NAME = loConductorData.CCATEGORY_NAME,
                CDISPLAY = "[" + loConductorData.ILEVEL + "] " + loConductorData.CCATEGORY_ID + " - " + loConductorData.CCATEGORY_NAME,
                ILEVEL = loConductorData.ILEVEL,
                //LHAS_CHILD = loConductorData.LH
            };
        }
