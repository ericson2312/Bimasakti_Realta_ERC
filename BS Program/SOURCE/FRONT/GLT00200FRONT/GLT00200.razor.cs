...
namespace GLT00200FRONT
{
    public partial class GLT00200 : R_Page
    {
        ...
        [Inject] private R_ILocalizer<GLT00200FrontResources.Resources_Dummy_Class> _localizer { get; set; }
        ...
        private async Task OnClickTemplate()
        {
            ...
            try
            {
                var loValidate = await R_MessageBox.Show("", _localizer["M003"], R_eMessageBoxButtonType.YesNo);
        ...
        private async Task OnClickImport()
        {
            ...
            try
            {
                ...
                var loB3CellValue = (double)Excel.R_ReadValueCellFromExcel(loImportJournalViewModel.loFileByte, "B3");

                if (loB3CellValue != null && double.TryParse(loB3CellValue.ToString(), out double excelDateSerialNumber1))
                {
                    DateTime dateTimeValue = DateTime.FromOADate(excelDateSerialNumber1);
                    loTempHeader.CREF_DATE = dateTimeValue.ToString("dd-MMM-yyyy");
                }
                else
                {
                ...
                var loB5CellValue = (double)Excel.R_ReadValueCellFromExcel(loImportJournalViewModel.loFileByte, "B5");

                if (loB3CellValue != null && double.TryParse(loB3CellValue.ToString(), out double excelDateSerialNumber2))
                {
                    DateTime dateTimeValue = DateTime.FromOADate(excelDateSerialNumber2);
                    loTempHeader.CDOC_DATE = dateTimeValue.ToString("dd-MMM-yyyy");
                }
                else
                {
                ...
                //DateTime loRefDate = DateTime.FromOADate(excelDateSerialNumber1);
                //DateTime loRefDate = DateTime.ParseExact(loTempHeader.CREF_DATE, "dd/MM/yyyy", System.Globalization.CultureInfo.InvariantCulture);
                ...
                //DateTime loDocDate = DateTime.ParseExact(loTempHeader.CDOC_DATE, "dd/MM/yyyy", System.Globalization.CultureInfo.InvariantCulture);
                //DateTime loDocDate = Convert.ToDateTime(loTempHeader.CDOC_DATE);
                //double excelDateSerialNumber2 = (double)Excel.R_ReadValueCellFromExcel(loImportJournalViewModel.loFileByte, "B3");
                //DateTime loDocDate = DateTime.FromOADate(excelDateSerialNumber2);
