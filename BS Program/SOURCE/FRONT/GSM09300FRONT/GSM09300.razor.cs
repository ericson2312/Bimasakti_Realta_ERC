...
namespace GSM09300FRONT
{
    ...
    {
        [Inject] private R_ILocalizer<GSM09300FrontResources.Resources_Dummy_Class> _localizer { get; set; }

        private GSM09300ViewModel loSupplierCategoryViewModel = new();

        private R_Conductor _conductorSupplierCategoryRef;

        private R_TreeView<GSM09300DTO> _treeSupplierCategoryRef;
        ...
        private R_TextBox _supplierIdRef;

        private R_TextBox _supplierNameRef;
        ...
        private async Task PropertyDropdown_ValueChanged(string poParam)
        {
            ...
            try
            {
                ...
                await _treeSupplierCategoryRef.R_RefreshTree(null);
        ...
        private async Task Grid_SupplierCategory_Display(R_DisplayEventArgs eventArgs)
        {
            ...
            {
                await _supplierNameRef.FocusAsync();
            }
        }

        private async Task Tree_SupplierCategory_R_ServiceGetListRecord(R_ServiceGetListRecordEventArgs eventArgs)
        {
            R_Exception loEx = new R_Exception();
        ...
        private void Tree_R_RefreshTreeViewState(R_RefreshTreeViewStateEventArgs eventArgs)
        {
            ...
            try
            {
                List<GSM09300DTO> loTreeList = (List<GSM09300DTO>)eventArgs.TreeViewList;
                loTreeList.ForEach(x => x.LHAS_CHILD = loTreeList.Where(y => y.CPARENT == x.CCATEGORY_ID).Count() > 0 ? true : false);
                eventArgs.ExpandedList = loSupplierCategoryViewModel.loSupplierCategoryList.Where(x => x.LHAS_CHILD == true).ToList();
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
        ...
        private async Task Grid_SupplierCategory_AfterAdd(R_AfterAddEventArgs eventArgs)
        {
            ...
            try
            {
                await _supplierIdRef.FocusAsync();
        ...
        private void Grid_SupplierCategory_ConvertToGridEntity(R_ConvertToGridEntityEventArgs eventArgs)
        {
            GSM09300DetailDTO loConductorData = (GSM09300DetailDTO)eventArgs.Data;
            eventArgs.GridData = new GSM09300DTO
            {
