...
namespace APM00200FRONT
{
    public partial class APM00200 : R_Page
    {
        ...
        [Inject] private R_ILocalizer<APM00200FrontResources.Resources_Dummy_Class> _localizer { get; set; }
        ...
        private bool IsActiveInactiveEnabled = false;
        ...
        private async Task ActivateDeactivateOnClick()
        {
            ...
            try
            {
                R_eMessageBoxResult loValidate = await R_MessageBox.Show("", String.Format(_localizer["M001"], loLabel), R_eMessageBoxButtonType.YesNo);
                ...
                if (loValidate == R_eMessageBoxResult.Yes)
                {
                    ...
                    if (loEx.HasError == false)
                    {
                        ...
                        loValidate = await R_MessageBox.Show("", String.Format(_localizer["M002"], lcOldLabel), R_eMessageBoxButtonType.OK);
                    }
                }
        ...
        private void WithholdingTaxCheckBox_ValueChanged(bool poParam)
        {
            ...
            try
            {
                ...
                if (_conGridExpenditureRef.R_ConductorMode == R_eConductorMode.Add || _conGridExpenditureRef.R_ConductorMode == R_eConductorMode.Edit)
                {
                    ...
                    if (loExpenditureViewModel.Data.LWITHHOLDING_TAX == false)
                    {
                        IsWitholdingTaxIdEnabled = false;
                    }
                }
        ...
        private void Grid_BeforeCancel(R_BeforeCancelEventArgs eventArgs)
        {
            ...
            IsWitholdingTaxIdEnabled = false;
            if (loExpenditureViewModel.loExpenditureList.Count > 0)
            {
                IsActiveInactiveEnabled = true;
            }
            else
            {
                IsActiveInactiveEnabled = false;
            }
        }
        ...
        private async Task Grid_Display(R_DisplayEventArgs eventArgs)
        {
            if (eventArgs.ConductorMode == R_eConductorMode.Normal)
            {
                ...
                if (loParam.LACTIVE)
                {
                    loLabel = _localizer["_Deactivate"];
                }
                else
                {
